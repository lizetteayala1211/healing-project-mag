export const navItems = [
  ...
  },
  {
    ...
    label: "Call & Response",
    byline:
      "Written and visual offerings by community members from Parole Prep, Brotherhood Sister Sol, The Fortune Society, and South Bronx Unite/The Land Stewards.",
  ...
  },
  {
    ...
    byline:
      "Invitations from each of the <br> aforementioned New York-based <br> organizations to join their work.",
  ...
  },
  {
    ...
    label: "Processing",
    byline:
      "A reflection by Mahogany L. Browne on <br> this issue’s theme, counter-narratives, and what it means to tell the whole truth.",
  ...
  },
  {
    ...
    label: "Collaborators",
    byline:
      "A list of this issue’s contributors <br> across editorial, design, and artwork.",
...
// ALWAYS compute mobile at time of use (not once at file-load),
// because resizing / device rotation can change it.
export const isMobile = () => window.matchMedia("(max-width: 1023px)").matches;

// Convert <br>, <br/>, <br /> to spaces + collapse extra whitespace.
export const cleanByline = (str = "") =>
  String(str)
    .replace(/<br\s*\/?>/gi, " ")
    .replace(/\s+/g, " ")
    .trim();

// Use this when you render the byline.
// Desktop keeps <br> formatting, mobile gets flattened.
export const formatByline = (byline = "") =>
  isMobile() ? cleanByline(byline) : byline;

// If you need a guaranteed "plain text" version always:
export const bylinePlain = (byline = "") => cleanByline(byline);

// Desktop-only rail toggle button:
export const shouldRenderRailToggle = () => !isMobile();

// Optional: a precomputed version of navItems that includes a mobile-safe byline.
// Use this if you want to avoid calling formatByline() inside render loops.
export const navItemsFormatted = () =>
  navItems.map((item) => ({
    ...item,
    bylineHtml: formatByline(item.byline),
    bylineText: bylinePlain(item.byline),
  }));
